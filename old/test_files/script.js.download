(function () {
    ...
    ready(function () {
        if (document.querySelectorAll('.mbr-arrow-up').length) {
            ...
            window.addEventListener('scroll', function () {
                ...
                // why not (window.scrollY > window.innerHeight)?
                if (scrollTop > scrollHeight / 2 - document.documentElement.clientHeight / 2) {
                    fadeIn(scroller);
                } else {
                    fadeOut(scroller);
                }
    ...
    function setProgressElement (el, cardID, percent) {
        ...
        if (!el.closest('.card').getAttribute('style')) 
            el.closest('.card').setAttribute('style', '-webkit-flex-basis: auto; flex-basis: auto;');
    ...
    function updateCircleProgress (section, paramName) {
        if (!paramName.includes('progress') || paramName == 'progressCount') return;
